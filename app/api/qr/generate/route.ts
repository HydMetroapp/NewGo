...
export async function POST(request: NextRequest) {
  try {
    ...
    if (location) {
      const isValidLocation = await GeofencingService.validateLocationForEntry(stationId, location)
    ...
    if (type === 'entry') {
      ...
      if (!journeyId) {
        return NextResponse.json({ error: 'Journey ID required for exit QR' }, { status: 400 })
      }
