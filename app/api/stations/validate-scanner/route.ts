...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!scanner) {
      ...
      const isValidScanner =
        scannerId.includes(stationId) && (scannerId.includes('entry') || scannerId.includes('exit'))

      if (!isValidScanner) {
        return NextResponse.json({ error: 'Invalid scanner for this station' }, { status: 403 })
      }
    }
    ...
    return NextResponse.json({ error: 'Scanner validation failed' }, { status: 500 })
  }
}
