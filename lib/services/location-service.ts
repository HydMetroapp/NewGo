...
export class LocationService {
  ...
  static removeGeofenceCallback(
    callback: (stationId: string, event: 'enter' | 'exit') => void,
  ): void {
  ...
  static async findNearestStationWithDistance(): Promise<{
    station: Station
    distance: number
  } | null> {
    if (!this.currentLocation) return null
